...
//    }
      ...
      forall(y in Years: y>1)
      {
         //Carbon-free constraint, must run both of the next two lines
		   (CO2_total[y] * 0.2727 + CH4_total[y] * 0.74868) <= (CO2_total[y-1] * 0.2727 * 0.85 + CH4_total[y-1] * 0.74868 * 0.85); //reduce carbon emissions annually to get to zero by 2045
      }
      	CO2_total[26] <= 0; //carbon-free goal (excluding transportation) 
